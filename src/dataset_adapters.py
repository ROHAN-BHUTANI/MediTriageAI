...
class UnifiedPatientRecord:
    ...
    severity_label: Optional[int] = None
...
class NHAMCSAdapter(DatasetAdapter):
    ...
    def iter_records(self) -> Iterator[UnifiedPatientRecord]:
        for file in self.data_path.rglob("*.csv"):
            for chunk in pd.read_csv(file, chunksize=10000, low_memory=False, on_bad_lines="skip"):
                for _, row in chunk.iterrows():
                    complaint = None
                    if "REASON" in row: complaint = row["REASON"]
                    elif "RFV1" in row: complaint = row["RFV1"]
                    elif "text" in row: complaint = row["text"]
                    ...
                    split = row.get("split") if "split" in row and pd.notna(row.get("split")) else assign_split(text)
...
class ChatDoctorAdapter(DatasetAdapter):
    ...
    def iter_records(self) -> Iterator[UnifiedPatientRecord]:
        for file in self.data_path.rglob("*"):
            ...
            folder_split = None
            if "train" in file.parts: folder_split = "train"
            elif "val" in file.parts or "valid" in file.parts: folder_split = "val"
            elif "test" in file.parts: folder_split = "test"
...
class PMCPatientsAdapter(DatasetAdapter):
    ...
    def iter_records(self) -> Iterator[UnifiedPatientRecord]:
        for file in self.data_path.rglob("*.csv"):
            for chunk in pd.read_csv(file, chunksize=5000, on_bad_lines="skip", low_memory=False):
                for _, row in chunk.iterrows():
...
class L3CubeAdapter(DatasetAdapter):
    ...
    def iter_records(self) -> Iterator[UnifiedPatientRecord]:
        for file in self.data_path.rglob("*.txt"):
            folder_split = None
            if "train" in file.parts or "train" in file.name.lower(): folder_split = "train"
            elif "val" in file.parts or "val" in file.name.lower(): folder_split = "val"
            elif "test" in file.parts or "test" in file.name.lower(): folder_split = "test"

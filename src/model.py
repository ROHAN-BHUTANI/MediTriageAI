...
class JointLossWeights:
    def __init__(
        self,
        alpha_specialist: float = DEFAULT_ALPHA,
        beta_severity: float = DEFAULT_BETA,
    ):
...
class MediTriageTransformer(nn.Module):
    def __init__(
        self, encoder: XLMRobertaModel, hidden_dropout_prob: float = 0.1
    ) -> None:
        super().__init__()
    ...
    def forward(
        ...
        return self.classifier_specialist(cls_representation), self.classifier_severity(
            cls_representation
        )
...
class JointLoss(nn.Module):
    def __init__(
        ...
        self.specialist_cross_entropy = FocalLoss(
            weight=specialist_class_weights, gamma=2.0
        )
        self.severity_cross_entropy = FocalLoss(
            weight=severity_class_weights, gamma=2.0
        )
    ...
    def forward(
        ...
        specialist_loss = self.specialist_cross_entropy(
            specialist_logits, labels_specialist
        )
        severity_loss = self.severity_cross_entropy(severity_logits, labels_severity)
        joint_loss = (
            self.weight.alpha_specialist * specialist_loss
            + self.weight.beta_severity * severity_loss
        )
        return {

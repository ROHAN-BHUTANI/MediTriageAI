...
class Calibrator:
    ...
    def fit(
        self,
        val_spec_logits: torch.Tensor,
        val_spec_labels: torch.Tensor,
        val_sev_logits: torch.Tensor,
        val_sev_labels: torch.Tensor,
        output_dir: str,
    ):
        ...
        before_spec_ece = (
            expected_calibration_error(
                val_spec_labels.numpy(), torch.softmax(val_spec_logits, dim=1).numpy()
            )
        ...
        before_sev_ece = (
            expected_calibration_error(
                val_sev_labels.numpy(), torch.softmax(val_sev_logits, dim=1).numpy()
            )
        ...
        if len(val_spec_labels) > 0:
            ...
            optimizer = optim.LBFGS(
                [self.specialist_scaler.temperature], lr=0.01, max_iter=50
            )

            def spec_eval():
                optimizer.zero_grad()
                loss = nll_criterion(
                    self.specialist_scaler(val_spec_logits), val_spec_labels
                )
                loss.backward()
                return loss
        ...
        if len(val_sev_labels) > 0:
            nll_criterion = nn.CrossEntropyLoss()
            optimizer = optim.LBFGS(
                [self.severity_scaler.temperature], lr=0.01, max_iter=50
            )

            def sev_eval():
                optimizer.zero_grad()
                loss = nll_criterion(
                    self.severity_scaler(val_sev_logits), val_sev_labels
                )
                loss.backward()
                return loss
        ...
        after_spec_ece = (
            expected_calibration_error(
                ...
                torch.softmax(
                    self.specialist_scaler(val_spec_logits).detach(), dim=1
                ).numpy(),
            )
            if len(val_spec_labels) > 0
            else 0.0
        ...
        after_sev_ece = (
            expected_calibration_error(
                ...
                torch.softmax(
                    self.severity_scaler(val_sev_logits).detach(), dim=1
                ).numpy(),
            )
            if len(val_sev_labels) > 0
            else 0.0

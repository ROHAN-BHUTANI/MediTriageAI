...
def create_stratified_subset(
    ...
    try:
        ...
        warnings.warn(
            "Classes too small to strictly stratify, falling back to random sample."
        )
    ...
    if secondary_col and secondary_col in df.columns:
        for val in df[secondary_col].unique():
            ...
            added_count = (
                (df.loc[list(guaranteed_indices), secondary_col] == val).sum()
            ...
            if current_count + added_count < min_guarantee:
                ...
                pool = df[
                    (df[secondary_col] == val)
                    & (~df.index.isin(subset.index))
                    & (~df.index.isin(guaranteed_indices))
                ]
                sampled = pool.sample(n=min(shortfall, len(pool)), random_state=seed)
                guaranteed_indices.update(sampled.index.tolist())
    ...
    if guaranteed_indices:
        ...
        for idx, row in subset.iterrows():
            ...
            if (
                secondary_col
                and not pd.isna(s_val)
                and sec_counts[s_val] <= min_guarantee
            ):
                can_drop = False
        ...
        if len(subset_to_drop) >= num_to_drop:
            ...
            final_subset_indices = list(set(subset.index) - actually_dropped) + list(
                guaranteed_indices
            )
        else:
            # Fallback: just drop the first `num_to_drop` rows from the largest classes
            final_subset_indices = list(subset.index)[num_to_drop:] + list(
                guaranteed_indices
            )

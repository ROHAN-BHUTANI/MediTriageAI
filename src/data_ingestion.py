    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "labels_specialist": torch.tensor(
                row["specialist_label"], dtype=torch.long
            ),
...
def load_and_split_dataset(
    ...
    gss_val = GroupShuffleSplit(
        n_splits=1, test_size=relative_val_split, random_state=seed
    )
    train_idx, val_idx = next(
        gss_val.split(df_train_val, groups=df_train_val["patient_id"])
    )
...
def load_ood_dataset(
    filepath: str | Path, tokenizer: Any, batch_size: int = 16
) -> DataLoader:

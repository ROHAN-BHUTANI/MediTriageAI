...
class ExperimentManager:
    def __init__(self, config: TrainingConfig, dataset_path: str = "meditriage/data/processed/dataset.parquet"):
        self.config = config
    ...
    def generate_dataset_manifest(self) -> dict:
        ...
        fingerprint_hash = hashlib.sha256(head_bytes + str(file_size).encode()).hexdigest()
        
        for i in range(pq_file.num_row_groups):
            df = pq_file.read_row_group(i, columns=["split", "department", "triage_level"]).to_pandas()
            total_rows += len(df)
        ...
        manifest = {
            ...
            "dataset_build_timestamp": datetime.fromtimestamp(self.dataset_path.stat().st_mtime, tz=timezone.utc).isoformat()
        }
    ...
    def validate_or_create_manifest(self) -> str:
        ...
        if manifest_path.exists():
            ...
            if current_manifest["dataset_fingerprint"] != saved_manifest["dataset_fingerprint"]:
                raise ValueError("Dataset manifest validation failed: The dataset fingerprint has changed! Aborting to prevent corrupted resumes.")
            if current_manifest["total_rows"] != saved_manifest["total_rows"]:
                raise ValueError("Dataset manifest validation failed: Total rows mismatch!")
        ...
        return hashlib.sha256(json.dumps(current_manifest, sort_keys=True).encode()).hexdigest()

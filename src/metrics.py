...
class MaskedMetrics:
    ...
    def update(
        ...
        if spec_mask.any():
            ...
            valid_spec_probs = torch.softmax(torch.tensor(valid_spec_logits), dim=-1).numpy()
        ...
        if sev_mask.any():
            ...
            valid_sev_probs = torch.softmax(torch.tensor(valid_sev_logits), dim=-1).numpy()
    ...
    def compute(self, specialist_class_names: List[str], severity_class_names: List[str]) -> Dict[str, Any]:
    ...
    def _compute_task_metrics(
        ...
        try:
            auroc = roc_auc_score(y_true, y_prob, multi_class="ovr", labels=unique_labels)
        except ValueError:
            auroc = float('nan') # E.g., if a class is entirely missing from y_true
            
        # Classification Report (Per-class Precision, Recall, F1)
        report = classification_report(
            y_true, y_pred, labels=unique_labels, target_names=class_names, output_dict=True, zero_division=0
        )
    ...
    def export_artifacts(
        ...
        for task, names in [("specialist", specialist_names), ("severity", severity_names)]:
            ...
            if cm.size > 0:
                ...
                df_norm_cm.to_csv(os.path.join(output_dir, f"{task}_normalized_confusion_matrix.csv"))

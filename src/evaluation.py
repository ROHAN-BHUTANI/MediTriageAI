...
class EvaluationExporter:
            ...
            self.predictions.append(
                {
                    ...
                    "department_correct": (
                        bool(s_label == s_pred) if s_label != -1 else None
                    ),
                    ...
                    "severity_correct": (
                        bool(v_label == v_pred) if v_label != -1 else None
                    ),
    ...
    def export(self):
        ...
        if "department_correct" in df.columns:
            mis_mask = (df["department_correct"] == False) | (
                df["severity_correct"] == False
            )
            cor_mask = (df["department_correct"] == True) & (
                df["severity_correct"] == True
            )

            df[mis_mask].to_csv(
                os.path.join(self.output_dir, "misclassified.csv"), index=False
            )
            df[cor_mask].to_csv(
                os.path.join(self.output_dir, "correct.csv"), index=False
            )

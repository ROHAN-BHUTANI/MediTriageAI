...
def make_tracking_id(seed_id: str, variant_index: int) -> str:
    basis = f"{seed_id}::{variant_index}".encode()
...
def compute_grouped_split(
    ...
    *,
    ...
    if len(set(seed_ids)) != len(seed_ids):
        ...
        raise ValueError(
            f"seed_ids must be unique; found {len(duplicates)} duplicate(s), e.g. {duplicates[:5]!r}."
        )
    ...
    for split_name, fraction in {
        ...
        if round(n_seeds * fraction) == 0 and not allow_empty_split:
            raise InsufficientSeedsError(
                f"Requested {split_name} split rounds to 0 rows for {n_seeds} seeds."
            )
    ...
    assignments = (
        {sid: "train" for sid in train_ids}
        | {sid: "val" for sid in val_ids}
        | {sid: "test" for sid in test_ids}
    )
    return SplitResult(
...
def assign_rows_to_split(
    tracking_ids: list[str], split_result: SplitResult
) -> dict[str, SplitName]:
    ...
    for tracking_id in tracking_ids:
        ...
        if seed_id not in split_result.assignments:
            raise KeyError(
                f"tracking_id {tracking_id!r} derives from seed_id {seed_id!r}, which was not included in the split computation."
            )

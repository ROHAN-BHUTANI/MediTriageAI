...
from ..hinglish_perturbation import perturb_text, PerturbationResult
...
class HinglishPerturbation(TransformationPlugin):
    ...
    def apply(self, text: str, rng: random.Random) -> Tuple[str, Dict]:
        ...
        result: PerturbationResult = perturb_text(text, seed, substitution_rate=self.substitution_rate)
        metadata = {
            "plugin": self.name,
            "seed": seed,
            "substitutions": [

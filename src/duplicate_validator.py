...
class DuplicateValidator:
    ...
    def __init__(self, original_path: Path, synthetic_path: Path):
        ...
        self.report_path = Path("data/processed/enriched/duplicate_validation_report.csv")
        self.report_path.parent.mkdir(parents=True, exist_ok=True)

    def _load(self, path: Path) -> pd.DataFrame:
        return pd.read_csv(path)

    def _exact_duplicates(self, df1: pd.DataFrame, df2: pd.DataFrame, source1: str, source2: str):
        dup_records = []
        ...
        for _, row in df2.iterrows():
            ...
            if note in set1:
                ...
                dup_records.append({
                    ...
                    "matched_record_id": matched["id"].iloc[0] if "id" in matched.columns else "",
    ...
    def _near_duplicates(self, df1: pd.DataFrame, df2: pd.DataFrame, source1: str, source2: str, threshold: float = 0.9):
        ...
        records1 = [(idx, token_set(row.get("text", ""))) for idx, row in df1.iterrows()]
    ...
    def validate(self):
        ...
        records = []
        records.extend(self._exact_duplicates(original, synthetic, "original", "synthetic"))
        records.extend(self._near_duplicates(original, synthetic, "original", "synthetic"))
        with self.report_path.open("w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=["record_id", "duplicate_type", "source_dataset", "matched_record_id", "reason"])
            writer.writeheader()
            for r in records:
                writer.writerow(r)

...
def test_legacy_column_names():
    df1 = pd.DataFrame(
        [{"text": "A", "department_code": "Emergency", "severity_heuristic": "1"}]
    )
    ...
    df2 = pd.DataFrame(
        [{"text": "B", "specialist_label": "PEDS", "severity_label": "S2"}]
    )
...
def test_production_dataset_schema():
    ...
    assert list(result["department"].replace({float("nan"): None})) == [
        "ED",
        "ORTHO",
        None,
    ]
    assert list(result["triage_level"].replace({float("nan"): None})) == [
        "S1",
        None,
        "S2",
    ]

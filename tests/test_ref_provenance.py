...
def test_provenance_data_structures():
    env = ExecutionEnvironment(
        timestamp="2026", seed=42, ref_version="1.0", repository_version="2.0"
    )
    env.validate()

    with pytest.raises(ProvenanceValidationError):
        ExecutionEnvironment(
            timestamp="", seed=42, ref_version="1.0", repository_version="2.0"
        ).validate()
...
def test_provenance_pipeline_and_manifest(temp_output):
    ...
    pipeline = ProvenancePipeline(
        registry, experiment_id="EXP_PROV", output_dir=str(temp_output)
    )
    ...
    with open(prov_file, "r") as f:
        ...
        assert (
            data["fingerprints"]["MockFingerprint"]["payload"]["hashed_seed"]
            == "777_hash"
        )

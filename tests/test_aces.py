...
def test_static_fusion_fallback():
    ...
    config = EmergentPathTriageConfig(aces_fusion_mode="A0", latent_dim=16, dces_dropout=0.0)
...
def test_attention_fusion_a3_mode():
    ...
    config = EmergentPathTriageConfig(aces_fusion_mode="A3", latent_dim=16, aces_num_heads=2)
    model = ClinicalEvidenceSynthesizer(hidden_dim=32, config=config)
    ...
    # Gradients
    loss = out.symptom.sum() + out.anatomical.sum() + out.temporal.sum() + out.systemic.sum()
    loss.backward()
...
def test_attention_fusion_a1_mode():
    ...
    config = EmergentPathTriageConfig(aces_fusion_mode="A1", latent_dim=16, aces_num_heads=2)
    model = ClinicalEvidenceSynthesizer(hidden_dim=32, config=config)
    assert isinstance(model.fusion, AttentionFusion)

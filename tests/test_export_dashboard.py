...
def test_build_payload_includes_models_and_summary(tmp_path: Path, monkeypatch) -> None:
    ...
    (results_dir / "mbert" / "metrics.json").write_text(
        json.dumps(
            {
                "model_display_name": "mBERT",
                "specialist_macro_f1": 0.7,
                "severity_macro_f1": 0.6,
            }
        ),
        encoding="utf-8",
    )
...
def test_write_dashboard_json_creates_file(tmp_path: Path) -> None:
    ...
    result_path = exporter.write_dashboard_json(
        output_path=output_path, results_dir=tmp_path / "results", dataset_csv=csv_path
    )

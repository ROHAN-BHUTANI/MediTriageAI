...
class MockPlotProvider(BaseVisualizationProvider):
    def get_metadata(self) -> VisualizationMetadata:
        return VisualizationMetadata(name="MockPlot", version="1.0", description="Mock plot provider", dependencies=[])
...
def test_visualization_data_structures():
    req = VisualizationRequest(experiment_id="E1", output_dir=".", metric_report_dict={})
    req.validate()
    
    with pytest.raises(VisualizationValidationError):
        VisualizationRequest(experiment_id="", output_dir=".", metric_report_dict={}).validate()
...
def test_provider_interface_and_deterministic_exports(temp_output):
    ...
    req = VisualizationRequest(experiment_id="EXP_TEST", output_dir=str(temp_output), metric_report_dict={})
...
def test_publication_figure_isolation(temp_output):
    ...
    req = VisualizationRequest(experiment_id="EXP_PUB", output_dir=str(temp_output), metric_report_dict={})
...
def test_visualization_registry_and_config():
    ...
    from ref.visualization.providers import ROCProvider, PrecisionRecallProvider
...
def test_visualization_pipeline(temp_output):
    ...
    req = VisualizationRequest(experiment_id="EXP_PIPE", output_dir=str(temp_output), metric_report_dict={})

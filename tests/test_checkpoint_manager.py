...
def test_save_and_load_checkpoint_v2(tmp_path):
    ...
    assert torch.equal(loaded["state_dict"]["classifier_specialist.weight"], state_dict["classifier_specialist.weight"])
    # Backward compatibility key check
    assert "model_state_dict" in loaded
    assert torch.equal(loaded["model_state_dict"]["classifier_specialist.weight"], state_dict["classifier_specialist.weight"])
...
def test_detect_model_class_legacy_patterns():
    ...
    assert detect_model_class_from_state_dict(distil_state) == "DistilBertMultilingualModel"
    ...
    assert detect_model_class_from_state_dict(xlmr_large_state) == "XLMRobertaLargeModel"
...
def test_load_legacy_raw_checkpoint(tmp_path):
    ...
    assert "model_state_dict" in loaded
    assert torch.equal(loaded["state_dict"]["classifier_specialist.weight"], legacy_state_dict["classifier_specialist.weight"])

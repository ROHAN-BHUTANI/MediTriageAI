    ...
    data = [
        {
            ...
            "text": (
                ...
                if i % 2 == 0
                else "Mera bahut chest pain ho rha he, cardiac problem ki shikayat he."
            ),
            "raw_medical_specialty": "Cardiovascular / Pulmonary",
            "department_code": (
                "CARDIO_PULM" if i < 15 else "GI"
            ),  # Introduce noisy boundary labels (similarity in GI)
...
def test_audit_execution(mock_dataset_csv, temp_output_dir):
    ...
    timestamp_dirs = [
        d for d in temp_output_dir.iterdir() if d.is_dir() and d.name != "latest"
    ]

...
def test_config_self_validation():
    ...
    with pytest.raises(
        ConfigurationError, match="latent_dim must be a positive multiple of 2"
    ):
        EmergentPathTriageConfig(latent_dim=65)

    # 4. Invalid temperature
    with pytest.raises(
        ConfigurationError, match="temperature must be strictly positive"
    ):
        EmergentPathTriageConfig(temperature=0.0)

    # 5. Negative loss weights
    with pytest.raises(
        ConfigurationError, match="Loss coefficient cons_lambda must be non-negative"
    ):
...
def test_loss_hook_integration():
    ...
    loss_dict = apply_loss_hook(
        built_model, spec_logits, sev_logits, labels_spec, labels_sev, loss_fn
    )
...
def test_dcrr_probability_normalization_and_shapes():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=16, num_thought_blocks=6, max_path_depth=3
    )
    ...
    assert torch.all(
        (decision.routing_probabilities == 0.0)
        | (decision.routing_probabilities == 1.0)
    )
...
def test_dcrr_deterministic_inference():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=16, num_thought_blocks=4, max_path_depth=3
    )
    from models.emergent_path_triage.dcrr import ClinicalReasoningRouter
...
def test_dcrr_stochastic_training():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, num_thought_blocks=4, max_path_depth=2
    )
    from models.emergent_path_triage.dcrr import ClinicalReasoningRouter
    ...
    assert torch.any(
        (decision.routing_probabilities > 0.0) & (decision.routing_probabilities < 1.0)
    )
...
def test_dcrr_serialization_and_cpu():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, num_thought_blocks=3, max_path_depth=2
    )
    from models.emergent_path_triage.dcrr import ClinicalReasoningRouter
...
def test_engine_correct_execution_order_and_skips():
    ...
    evidence = [torch.ones(1, 8), torch.ones(1, 8), torch.ones(1, 8), torch.ones(1, 8)]
...
def test_engine_invalid_routing_and_validation():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, num_thought_blocks=3, max_path_depth=2, ctb_hidden_dim=16
    )
    from models.emergent_path_triage.ctb import ClinicalThoughtBlock
    from models.emergent_path_triage.engine import ReasoningPathExecutionEngine
    ...
    blocks_too_few = nn.ModuleList([ClinicalThoughtBlock(latent_dim=8, config=config)])
...
def test_head_output_dimensions():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=16, head_hidden_dim=32, head_dropout=0.0
    )
...
def test_head_gradient_propagation():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, head_hidden_dim=16, head_dropout=0.0
    )
    from models.emergent_path_triage.heads import PredictionHead
...
def test_loss_dcp_consistency_correctness():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, cons_lambda=1.0, ortho_lambda=0.0, div_lambda=0.0
    )
    from src.model import JointLoss
...
def test_loss_routing_diversity_correctness():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, div_lambda=1.0, cons_lambda=0.0, ortho_lambda=0.0
    )
    from src.model import JointLoss
...
def test_loss_orthogonality_correctness():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, ortho_lambda=1.0, cons_lambda=0.0, div_lambda=0.0
    )
    from src.model import JointLoss
    ...
    model._last_evidence = EvidenceRepresentation(
        symptom=ones, anatomical=ones, temporal=ones, systemic=ones
    )
...
def test_composite_loss_weights_and_zero_weight():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, ortho_lambda=0.0, cons_lambda=0.0, div_lambda=0.0
    )
    from src.model import JointLoss
    ...
    expected = (
        config.alpha_specialist * res["specialist_loss"]
        + config.beta_severity * res["severity_loss"]
    )
...
def test_loss_gradient_propagation():
    ...
    config = EmergentPathTriageConfig(
        latent_dim=8, ortho_lambda=1.0, cons_lambda=1.0, div_lambda=1.0
    )
    from src.model import JointLoss
...
def test_trainer_lifecycle_and_checkpointing():
    ...
    resumed_epoch = new_trainer.load_checkpoint(
        Path(temp_checkpoint_dir) / "latest_model.pt"
    )

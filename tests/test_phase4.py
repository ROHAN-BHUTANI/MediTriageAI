...
from src.calibration import Calibrator, expected_calibration_error
...
def test_prediction_exports(tmp_path):
    ...
    exporter.add_batch(
        ["s1", "s2"], ["train", "train"], ["src", "src"], ["en", "en"],
...
def test_calibration(tmp_path):
    ...
    report = calibrator.fit(spec_logits, spec_labels, sev_logits, sev_labels, str(tmp_path))
...
def test_artifact_integrity(tmp_path):
    ...
    exporter.add_batch(
        ...
        torch.tensor([-1]), torch.tensor([-1]) # masked labels
    )
    exporter.export()

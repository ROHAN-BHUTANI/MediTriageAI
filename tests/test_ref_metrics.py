...
class MockMetricProvider(BaseMetricProvider):
    def get_metadata(self) -> MetricMetadata:
        return MetricMetadata(name="Mock", version="1.0", description="Mock provider", dependencies=[])
...
def test_metric_aggregation_pipeline():
    ...
    assert report.groups["test_group"].collections["Mock"].results["computed_val"].value == 40.0

...
def mock_audit_dir(tmp_path):
    ...
    with open(audit_path / "augmentation_recommendations.json", "w") as f:
        json.dump({
            "GI": {"recommended_augmentations": ["typo augmentation", "paraphrases"]},
            "GEN_MED": {"recommended_augmentations": ["ASR augmentation"]},
            "CARDIO_PULM": {"recommended_augmentations": ["more English samples", "Hindi samples", "Hinglish samples"]}
        }, f)
...
def test_dry_run_mode(mock_dataset_csv, mock_audit_dir, temp_output_dir):
    cmd = [
        sys.executable, "scripts/dataset_quality_improvement.py",
        "--dataset", mock_dataset_csv,
        "--audit-dir", mock_audit_dir,
...
def test_production_improvement_run(mock_dataset_csv, mock_audit_dir, temp_output_dir):
    ...
    assert t3_row["text"] == "SUBJECTIVE: Patient presents with cough and fever-high temp."

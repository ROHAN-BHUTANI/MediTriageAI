...
def test_benchmark_data_structures():
    bd = BenchmarkDefinition(
        name="task1", target_metric="f1", baseline_experiment_id="base"
    )
    bd.validate()

    with pytest.raises(BenchmarkValidationError):
        BenchmarkDefinition(
            name="", target_metric="f1", baseline_experiment_id=""
        ).validate()
    ...
    comp = BenchmarkComparison(
        comparison_id="c1", base_run=br, candidate_runs=[br], deltas={"d": 0}
    )
    comp.validate()
...
def test_benchmark_pipeline_and_manifest(temp_output):
    ...
    pipeline = BenchmarkPipeline(
        registry, report_id="REP_TEST", output_dir=str(temp_output)
    )

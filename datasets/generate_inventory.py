...
expected = {
    ...
    "chatdoctor_healthcaremagic": {
        "desc": "ChatDoctor HealthCareMagic",
        "lic": "Research Use Only",
    },
    ...
    "disease_symptom_description": {
        "desc": "Disease Symptom Description",
        "lic": "Open Access",
    },
}
...
for name, info in expected.items():
    ...
    if fc > 0:
        ...
        if name == "meddialog_en" and any(
            p.name == "DOWNLOAD_INSTRUCTIONS.txt" for p in d.iterdir()
        ):
            status = "SKIPPED"
...
with open(INV_PATH, "w", encoding="utf-8") as f:
    ...
    if downloaded:
        ...
        for name, info, fc, tb in downloaded:
            size_str = f"{tb:,} bytes"
            f.write(
                f"| **{name}** | {info['lic']} | {fc} | {size_str} | {info['desc']} |\n"
            )
    else:
    ...
    f.write(
        "The following datasets are highly relevant to MediTriageAI but require manual registration, "

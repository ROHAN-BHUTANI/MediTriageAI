...
def src(d, url):
    with open(d / "SOURCE_URL.txt", "w") as f:
        f.write(url + "\n")
...
if not (d / "mtsamples.csv").exists():
    ...
    if data:
        src(d, urls[0])
        lic_file("mtsamples", "CC0 Public Domain\nSource: Kaggle/mtsamples.com\n")
        fc, tb = count(d)
        write_meta("mtsamples", urls[0], "CC0", fc, tb, sha256(data))
        results.append(("mtsamples", "DOWNLOADED", fc, tb))
    else:
        results.append(("mtsamples", "FAILED", 0, 0))
else:
    fc, tb = count(d)
    results.append(("mtsamples", "EXISTS", fc, tb))
    log("  Already exists")
...
if not any(d.glob("*.csv")) and not any(d.glob("*.json")):
    ...
    if data:
        ...
        fc, tb = count(d)
        write_meta("pmc_patients", url, "CC BY-NC-SA 4.0", fc, tb, sha256(data))
    ...
    fc, tb = count(d)
    results.append(("pmc_patients", "EXISTS", fc, tb))

# ========== 3. ChatDoctor HealthCareMagic ==========
log("\n== ChatDoctor-HealthCareMagic ==")
d = RAW / "chatdoctor_healthcaremagic"
d.mkdir(parents=True, exist_ok=True)
if not any(d.iterdir()):
    ...
    if data:
        ...
        fc, tb = count(d)
        write_meta(
            "chatdoctor_healthcaremagic", url, "Research Use", fc, tb, sha256(data)
        )
    ...
    fc, tb = count(d)
    results.append(("chatdoctor_healthcaremagic", "EXISTS", fc, tb))

# ========== 4. ChatDoctor iCliniq ==========
log("\n== ChatDoctor-iCliniq ==")
d = RAW / "chatdoctor_icliniq"
d.mkdir(parents=True, exist_ok=True)
if not any(d.iterdir()):
    ...
    if data:
        ...
        fc, tb = count(d)
        write_meta("chatdoctor_icliniq", u, "Research Use", fc, tb, sha256(data))
    ...
    fc, tb = count(d)
    results.append(("chatdoctor_icliniq", "EXISTS", fc, tb))
...
if not any(d.iterdir()):
    ...
    with open(d / "DOWNLOAD_INSTRUCTIONS.txt", "w") as f:
        ...
        f.write(
            "Then use: load_dataset('medical_dialog', name='en', data_dir='<path>')\n"
        )
    log("  Requires manual Google Drive download; instructions saved")
else:
    fc, tb = count(d)
    results.append(("meddialog_en", "EXISTS", fc, tb))

# ========== 6. FedMML ED Triage ==========
log("\n== FedMML ED Triage ==")
d = RAW / "fedmml_ed_triage"
d.mkdir(parents=True, exist_ok=True)
if not any(p for p in d.iterdir() if p.suffix in (".parquet", ".csv")):
    # Try HF API to discover files
    api_url = (
        "https://huggingface.co/api/datasets/olaflaitinen/fedmml-ed-triage/tree/main"
    )
    try:
        ...
        if parquets:
            ...
            if any_ok:
                ...
                fc, tb = count(d)
                write_meta("fedmml_ed_triage", api_url, "CC BY 4.0", fc, tb)
    ...
    fc, tb = count(d)
    results.append(("fedmml_ed_triage", "EXISTS", fc, tb))
...
if not any(d.iterdir()):
    ...
    if data:
        src(d, url)
        lic_file(
            "disease_symptom", "Open/Research\nGitHub: itratrahman/disease_prediction\n"
        )
        fc, tb = count(d)
        write_meta("disease_symptom_description", url, "Open", fc, tb, sha256(data))
        results.append(("disease_symptom_description", "DOWNLOADED", fc, tb))
    else:
        results.append(("disease_symptom_description", "FAILED", 0, 0))
else:
    fc, tb = count(d)
    results.append(("disease_symptom_description", "EXISTS", fc, tb))

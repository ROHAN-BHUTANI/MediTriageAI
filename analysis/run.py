...
from analysis.calibration import compute_ece_mce, compute_nll, compute_brier_score, get_reliability_curve_data
...
from analysis.utils import setup_logger, set_seed, compute_sha256, compute_mcnemar_test, df_to_markdown
...
def run_pipeline() -> None:
    ...
    for model_name in config.models:
        predictions_dict[model_name] = generate_and_cache_predictions(model_name, config, logger)
    ...
    for model_name, df in enriched_dict.items():
        ...
        spec_acc_m, spec_acc_l, spec_acc_u = bootstrap_metric_ci(
            y_true_spec_int, y_pred_spec_int, spec_acc_fn, config.bootstrap_iterations, config.random_seed
        )
        spec_f1_m, spec_f1_l, spec_f1_u = bootstrap_metric_ci(
            y_true_spec_int, y_pred_spec_int, spec_f1_fn, config.bootstrap_iterations, config.random_seed
        )
        spec_w_f1_m, spec_w_f1_l, spec_w_f1_u = bootstrap_metric_ci(
            y_true_spec_int, y_pred_spec_int, spec_w_f1_fn, config.bootstrap_iterations, config.random_seed
        )
        ...
        for _ in range(config.bootstrap_iterations):
            ...
            top3_resamples.append(spec_top3_fn(spec_probs[indices], spec_true_ids[indices]))
        ...
        sev_acc_m, sev_acc_l, sev_acc_u = bootstrap_metric_ci(
            y_true_sev_int, y_pred_sev_int, spec_acc_fn, config.bootstrap_iterations, config.random_seed
        )
        sev_f1_m, sev_f1_l, sev_f1_u = bootstrap_metric_ci(
            y_true_sev_int, y_pred_sev_int, sev_f1_fn, config.bootstrap_iterations, config.random_seed
        )
        sev_w_f1_m, sev_w_f1_l, sev_w_f1_u = bootstrap_metric_ci(
            y_true_sev_int, y_pred_sev_int, sev_w_f1_fn, config.bootstrap_iterations, config.random_seed
        )
    ...
    (tables_dir / "overall_metrics.md").write_text(df_to_markdown(summary_df), encoding="utf-8")
    ...
    for model_name, df in enriched_dict.items():
        spec_class_df = compute_per_class_metrics(df["true_specialist"].values, df["pred_specialist"].values, SPECIALIST_CLASSES)
        sev_class_df = compute_per_class_metrics(df["true_severity"].values, df["pred_severity"].values, SEVERITY_LABELS)
        
        spec_class_df.to_csv(tables_dir / f"{model_name}_specialist_per_class_metrics.csv", index=False)
        sev_class_df.to_csv(tables_dir / f"{model_name}_severity_per_class_metrics.csv", index=False)
    ...
    for model_name, df in enriched_dict.items():
        ...
        spec_cm = confusion_matrix(df["true_specialist"].values, df["pred_specialist"].values, labels=SPECIALIST_CLASSES)
        plot_confusion_matrix(
            spec_cm, SPECIALIST_CLASSES, f"{model_name} Specialist Confusion Matrix",
            figures_dir / f"{model_name}_specialist_confusion.png", config.plot_dpi
        )
        
        # Severity confusion
        sev_cm = confusion_matrix(df["true_severity"].values, df["pred_severity"].values, labels=SEVERITY_LABELS)
        plot_confusion_matrix(
            sev_cm, SEVERITY_LABELS, f"{model_name} Severity Confusion Matrix",
            figures_dir / f"{model_name}_severity_confusion.png", config.plot_dpi
        )
    ...
    for model_name, df in enriched_dict.items():
        ...
        if not spec_wrong.empty:
            counts = spec_wrong.groupby(["true_specialist", "pred_specialist"]).size().reset_index(name="count")
            ...
            for _, r in counts.iterrows():
                confused_spec_rows.append({
                    ...
                    "Percentage of Errors (%)": (r["count"] / len(spec_wrong)) * 100
                })
        
        # Severity errors
        sev_wrong = df[df["true_severity"] != df["pred_severity"]]
        if not sev_wrong.empty:
            counts_sev = sev_wrong.groupby(["true_severity", "pred_severity"]).size().reset_index(name="count")
    ...
    for model_name, df in enriched_dict.items():
        ...
        spec_true_ids = np.array([SPECIALIST_CLASSES.index(x) for x in df["true_specialist"].values])
        sev_true_ids = np.array([SEVERITY_LABELS.index(x) for x in df["true_severity"].values])
        
        # Specialist calibration
        spec_ece, spec_mce = compute_ece_mce(spec_probs, spec_true_ids)
        spec_brier = compute_brier_score(spec_probs, spec_true_ids, len(SPECIALIST_CLASSES))
        ...
        plot_reliability_diagram(
            spec_rel, spec_ece, f"{model_name} Specialist Calibration Curve",
            figures_dir / f"{model_name}_specialist_reliability.png", config.plot_dpi
        )
        
        # Save confidence histogram
        plot_confidence_histogram(
            np.max(spec_probs, axis=1), np.mean(np.max(spec_probs, axis=1)), np.mean(df["spec_correct"]),
            f"{model_name} Specialist Confidence Distribution",
            figures_dir / f"{model_name}_specialist_conf_histogram.png", config.plot_dpi
        )

        # Severity diagrams
        sev_rel = get_reliability_curve_data(sev_probs, sev_true_ids)
        plot_reliability_diagram(
            sev_rel, sev_ece, f"{model_name} Severity Calibration Curve",
            figures_dir / f"{model_name}_severity_reliability.png", config.plot_dpi
        )
        plot_confidence_histogram(
            np.max(sev_probs, axis=1), np.mean(np.max(sev_probs, axis=1)), np.mean(df["sev_correct"]),
            f"{model_name} Severity Confidence Distribution",
            figures_dir / f"{model_name}_severity_conf_histogram.png", config.plot_dpi
        )
        
    calibration_df = pd.DataFrame(calibration_rows)
    calibration_df.to_csv(tables_dir / "calibration_metrics.csv", index=False)
    (tables_dir / "calibration_metrics.md").write_text(df_to_markdown(calibration_df), encoding="utf-8")
    ...
    for model_name, df in enriched_dict.items():
        for lang in heuristic_langs:
            ...
            count = len(df_lang)
            if count > 0:
                spec_acc = np.mean(df_lang["true_specialist"] == df_lang["pred_specialist"])
    ...
    (tables_dir / "language_metrics.md").write_text(df_to_markdown(lang_df), encoding="utf-8")
    ...
    for model_name, df in enriched_dict.items():
        for b_name, b_filter in buckets:
            ...
            count = len(df_bucket)
            if count > 0:
                spec_acc = np.mean(df_bucket["true_specialist"] == df_bucket["pred_specialist"])
                sev_acc = np.mean(df_bucket["true_severity"] == df_bucket["pred_severity"])
                len_rows.append({
                    "Model": model_name,
    ...
    (tables_dir / "length_metrics.md").write_text(df_to_markdown(len_df), encoding="utf-8")
    ...
    logger.info(f"Rare specialist classes identified (support <= {q25}): {rare_classes}")
    ...
    (tables_dir / "rare_class_metrics.md").write_text(df_to_markdown(rare_df), encoding="utf-8")
    logger.info("Calculated metrics for rare/minority departments.")

    # 13. Cross-Model Agreement (percentage agreement & Cohen's Kappa)
    spec_kappa_df, spec_pct_df = compute_pairwise_agreement(enriched_dict, "pred_specialist")
    sev_kappa_df, sev_pct_df = compute_pairwise_agreement(enriched_dict, "pred_severity")
    ...
    plot_agreement_heatmap(spec_kappa_df, "Specialist Pairwise Agreement (Cohen's Kappa)", figures_dir / "specialist_agreement_kappa.png", config.plot_dpi)
    plot_agreement_heatmap(sev_kappa_df, "Severity Pairwise Agreement (Cohen's Kappa)", figures_dir / "severity_agreement_kappa.png", config.plot_dpi)
    ...
    logger.info(f"Exported {len(df_failures)} total model failure cases to failure_cases.csv.")

    # 16. Compile Markdown and HTML reports
    md_path, html_path = generate_reports(
        exp_dir, summary_df, class_metrics_dict, taxonomy_dict,
        calibration_df, {
            "spec_kappa": spec_kappa_df, "spec_pct": spec_pct_df,
            "sev_kappa": sev_kappa_df, "sev_pct": sev_pct_df
        },
        lang_df, len_df, rare_df, mcnemar_results, config, logger
    )
    ...
    reports_exist = (reports_dir / "analysis_report.md").exists() and (reports_dir / "analysis_report.html").exists()
    
    # Check predictions cache availability
    missing_caches = [m for m in config.models if not config.get_prediction_cache_path(m).exists()]
    ...
    if manifest_path.exists():
        print("[PASS] Research manifest containing SHA256 checksums successfully exported.")
    else:

...
def plot_confusion_matrix(
    cm: np.ndarray, classes: list[str], title: str, save_path: Path, dpi: int = 300
) -> None:
...
def plot_confidence_histogram(
    ...
    ax.axvline(
        ...
        linestyle="-",
        linewidth=1.5,
        label=f"Mean Conf ({mean_conf:.3f})",
    )
    ax.axvline(
        accuracy,
        color="green",
...
def plot_agreement_heatmap(
    df_matrix: pd.DataFrame, title: str, save_path: Path, dpi: int = 300
) -> None:
...
def plot_dataset_distributions(
    df_test: pd.DataFrame, figures_dir: Path, dpi: int = 300
) -> dict[str, Path]:
    ...
    ax.fill_between(
        range(len(sorted_counts)), sorted_counts.values, color="#d62728", alpha=0.1
    )
    ax.set_title(
    ...
    if not rare_counts.empty:
        sns.barplot(
            ...
            hue=rare_counts.index,
            legend=False,
            palette="magma",
            ax=ax,
        )
    ax.set_title(
        f"Rare-Class Specialist Distribution (Support <= {int(q25)})",
        fontsize=12,
        fontweight="bold",
        pad=12,
    )

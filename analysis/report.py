...
def generate_reports(
    ...
    for model in config.models:
        ...
        md_content.append(
            f"![{model} Specialist Calibration]({fig_rel_path}/{model}_specialist_reliability.png)"
        )
        md_content.append(
            f"![{model} Specialist Confidence Histogram]({fig_rel_path}/{model}_specialist_conf_histogram.png)"
        )
        md_content.append(
            f"![{model} Severity Calibration]({fig_rel_path}/{model}_severity_reliability.png)"
        )
        md_content.append(
            f"![{model} Severity Confidence Histogram]({fig_rel_path}/{model}_severity_conf_histogram.png)\n"
        )
    ...
    md_content.append(
        f"![Severity Kappa Agreement]({fig_rel_path}/severity_agreement_kappa.png)\n"
    )
...
"""
    ...
    html_content.append(
        "<h1>MediTriageAI: Comprehensive Error Analysis & Model Calibration Report</h1>"
    )
    html_content.append(
        f"<p><em>Generated automatically for publication review. Experiment Directory: <code>{experiment_dir.name}</code></em></p>"
    )
    ...
    html_content.append(
        "<p>Values represent metrics with <strong>95% Bootstrap Confidence Intervals</strong> (1,000 resamples).</p>"
    )
    ...
    # Root images
    html_content_root.append(
        img_tag(
            f"{fig_rel_path}/class_frequency.png",
            "Dataset Specialist Class Frequencies (Test Split)",
        )
    )
    html_content_root.append(
        img_tag(
            f"{fig_rel_path}/long_tail_distribution.png",
            "Long-tail Specialist Label Distribution",
        )
    )
    html_content_root.append(
        img_tag(
            f"{fig_rel_path}/rare_class_distribution.png",
            "Rare-Class Specialist Distribution",
        )
    )

    # Exp images (inside reports/ folder, figures are in ../figures)
    html_content_exp.append(
        img_tag(
            "../figures/class_frequency.png",
            "Dataset Specialist Class Frequencies (Test Split)",
        )
    )
    html_content_exp.append(
        img_tag(
            "../figures/long_tail_distribution.png",
            "Long-tail Specialist Label Distribution",
        )
    )
    html_content_exp.append(
        img_tag(
            "../figures/rare_class_distribution.png",
            "Rare-Class Specialist Distribution",
        )
    )
    ...
    for model in config.models:
        ...
        html_content_root.append(
            img_tag(
                f"{fig_rel_path}/{model}_specialist_reliability.png",
                f"{model} Specialist Reliability Curve",
            )
        )
        html_content_root.append(
            img_tag(
                f"{fig_rel_path}/{model}_specialist_conf_histogram.png",
                f"{model} Specialist Confidence Histogram",
            )
        )
        html_content_root.append(
            img_tag(
                f"{fig_rel_path}/{model}_severity_reliability.png",
                f"{model} Severity Reliability Curve",
            )
        )
        html_content_root.append(
            img_tag(
                f"{fig_rel_path}/{model}_severity_conf_histogram.png",
                f"{model} Severity Confidence Histogram",
            )
        )
        ...
        html_content_exp.append(
            img_tag(
                f"../figures/{model}_specialist_reliability.png",
                f"{model} Specialist Reliability Curve",
            )
        )
        html_content_exp.append(
            img_tag(
                f"../figures/{model}_specialist_conf_histogram.png",
                f"{model} Specialist Confidence Histogram",
            )
        )
        html_content_exp.append(
            img_tag(
                f"../figures/{model}_severity_reliability.png",
                f"{model} Severity Reliability Curve",
            )
        )
        html_content_exp.append(
            img_tag(
                f"../figures/{model}_severity_conf_histogram.png",
                f"{model} Severity Confidence Histogram",
            )
        )
    ...
    common_part_2.append(
        "<p>We bucket clinical queries by word count to assess accuracy on short vs. descriptive texts.</p>"
    )
    common_part_2.append(len_metrics_df.to_html(index=False, classes="table"))
    common_part_2.append(
        "<p><em>Longer clinical descriptions provide richer vocabulary, leading to higher triage accuracy.</em></p>"
    )
    ...
    common_part_2.append(
        "<h2>8. Cross-Model Agreement &amp; Statistical Significance</h2>"
    )
    common_part_2.append(
    ...
    html_content_root.append(
        img_tag(
            f"{fig_rel_path}/specialist_agreement_kappa.png",
            "Specialist Kappa Agreement",
        )
    )
    html_content_root.append(
        img_tag(
            f"{fig_rel_path}/severity_agreement_kappa.png", "Severity Kappa Agreement"
        )
    )
    ...
    html_content_exp.append(
        img_tag(
            "../figures/specialist_agreement_kappa.png", "Specialist Kappa Agreement"
        )
    )
    html_content_exp.append(
        img_tag("../figures/severity_agreement_kappa.png", "Severity Kappa Agreement")
    )

...
def run_baseline_campaign(max_samples: int | None = 200) -> None:
    ...
    if not os.path.exists(data_config.dataset_path):
        raise FileNotFoundError(
            f"Processed dataset not found at {data_config.dataset_path}"
        )
    ...
    if max_samples is not None and len(df) > max_samples:
        print(
            f"Limiting dataset to {max_samples} samples for local campaign validation."
        )
    ...
    def process_df(target_df: pd.DataFrame) -> EmergentTriageDataset:
        ...
        spec_ids = [
            validator.validate_specialist(str(c)) for c in target_df["department_code"]
        ]
        sev_ids = [
            validator.validate_severity(str(l)) for l in target_df["severity_heuristic"]
        ]
    ...
    train_loader = get_dataloader(
        train_ds, batch_size=data_config.batch_size, shuffle=True
    )
    val_loader = get_dataloader(
        val_ds, batch_size=data_config.batch_size, shuffle=False
    )
    test_loader = get_dataloader(
        test_ds, batch_size=data_config.batch_size, shuffle=False
    )
    ...
    plt.plot(
        history_df["epoch"], history_df["train_loss"], label="Train Loss", marker="o"
    )
    ...
    plt.plot(
        history_df["epoch"],
        history_df["train_specialist_acc"],
        label="Train Specialist Acc",
        marker="o",
    )
    plt.plot(
        history_df["epoch"],
        history_df["val_specialist_acc"],
        label="Val Specialist Acc",
        marker="x",
    )
    plt.plot(
        history_df["epoch"],
        history_df["train_severity_acc"],
        label="Train Severity Acc",
        marker="s",
    )
    plt.plot(
        history_df["epoch"],
        history_df["val_severity_acc"],
        label="Val Severity Acc",
        marker="d",
    )

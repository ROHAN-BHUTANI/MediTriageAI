...
def main():
    ...
    surgery_other = all_df[(all_df['true_specialist_name'] == 'SURGERY') & (all_df['predicted_specialist_name'] != 'ORTHO')]
    ...
    print(f"SURGERY->Other Misclassified: {len(surgery_other)}")
    print(f"ORTHO Correctly Classified: {len(ortho_correct)}")
    
    surgery_ortho.to_csv(out_dir / "surgery_to_ortho.csv", index=False)
    surgery_other.to_csv(out_dir / "surgery_other.csv", index=False)
    ortho_correct.to_csv(out_dir / "ortho_correct.csv", index=False)

    # 3. TF-IDF Analysis
    vectorizer = TfidfVectorizer(max_features=500, stop_words='english')
    texts = pd.concat([surgery_ortho['original_complaint'], surgery_other['original_complaint']])
    if len(texts) == 0:
    ...
    for i in range(X.shape[1]):
        ...
        mean_mis = np.mean(mis_scores)
        mean_cor = np.mean(cor_scores)
        ...
        if p_val < 0.05 and not np.isnan(p_val):
            stats.append({
                ...
                "mean_tfidf_surgery_other": float(mean_cor),
                "diff": float(diff),
                "t_stat": float(t_stat),
                "p_value": float(p_val)
            })
    ...
    len_cor = surgery_other['original_complaint'].fillna('').str.len()
    ...
    route_cor = pd.to_numeric(surgery_other['routing_confidence'], errors='coerce').dropna()
    ...
    report.append(f"- **SURGERY -> Other Misclassified:** {len(surgery_other)}")
    report.append(f"- **ORTHO Correct:** {len(ortho_correct)}")
    
    report.append(f"\n## Length Analysis")
    report.append(f"- **Mean Length (ORTHO Misclassified):** {np.mean(len_mis):.1f} chars")
    report.append(f"- **Mean Length (Other Misclassified):** {np.mean(len_cor):.1f} chars")
    ...
    if len(route_mis) > 0 and len(route_cor) > 0:
        ...
        report.append(f"- **Mean Confidence (Other Misclassified):** {np.mean(route_cor):.3f}")
        report.append(f"- **Significance (p-value):** {p_val_route:.4e}")
    
    report.append("\n## Top 15 Discriminative Tokens (TF-IDF)")
    if not stats_df.empty:
        report.append("| Token | Mean (ORTHO Shortcut) | Mean (Other Misclassified) | Diff | p-value |")
        report.append("|---|---|---|---|---|")
        for _, row in stats_df.head(15).iterrows():
            report.append(f"| {row['token']} | {row['mean_tfidf_surgery_ortho']:.4f} | {row['mean_tfidf_surgery_other']:.4f} | {row['diff']:.4f} | {row['p_value']:.4e} |")
    else:

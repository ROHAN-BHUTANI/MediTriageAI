...
class DatasetQualityImprovementEngine:
    def __init__(self, config: dict, cli_args: argparse.Namespace):
        ...
        self.output_dir = Path(self.config.get("improvement", {}).get("output_dir", "data/processed/improved"))
    ...
    def execute(self):
        ...
        self.logger.info(f"Mode: {'DRY RUN (no files modified)' if self.dry_run else 'PRODUCTION CLEANING'}")
        self.logger.info("==================================================")
        ...
        if not dataset_path.exists():
            ...
            sys.exit(1)
        dataset_checksum_before = calculate_sha256(dataset_path)
        ...
        score_before, score_after, compare_metrics = self.compute_quality_comparison(df_orig, df, audit_data, label_review_df)
        ...
        self.export_artifacts(df, label_review_df, aug_plan, merge_plan_content, compare_metrics, score_before, score_after)
        ...
        self.logger.info(f"Quality Score : {score_before['total']:.2f}/100 -> {score_after['total']:.2f}/100")
        self.logger.info("==================================================")
    ...
    def load_audit_outputs(self) -> dict:
        ...
        for fn, key in filenames.items():
            ...
            if not path.exists():
                self.logger.warning(f"Required audit output file '{path}' not found. Initializing empty fallback.")
                if fn.endswith(".csv"):
    ...
    def clean_and_normalize(self, df: pd.DataFrame, audit_data: dict) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            ...
            if pd.isna(tid) or pd.isna(text) or not isinstance(text, str):
                ...
                self.review_actions[str(tid) if pd.notna(tid) else f"row_{idx}"] = ("REMOVE", "Malformed record")
                continue
            ...
            if len(text.strip()) <= min_length:
                ...
                self.review_actions[tid_str] = ("REMOVE", "Insufficient complaint length")
                continue
    ...
    def detect_suspicious_labels(self, df: pd.DataFrame, audit_data: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
        inflow_count = len(df)
        review_candidates = []
            ...
            try:
                ...
                if not noisy_labels_df.empty:
                    if "tracking_id_1" in noisy_labels_df.columns:
                        noisy_ids.update(noisy_labels_df["tracking_id_1"].astype(str).tolist())
                    if "tracking_id_2" in noisy_labels_df.columns:
                        noisy_ids.update(noisy_labels_df["tracking_id_2"].astype(str).tolist())
                ...
                for i, row in df.iterrows():
                    ...
                    is_suspicious = (pred_class != annotated) and (confidence >= threshold)
                    ...
                    if is_suspicious or in_noisy_audit:
                        reason_parts = []
                        if is_suspicious:
                            reason_parts.append(f"Model predicts '{pred_class}' with {confidence:.2f} probability")
                        if in_noisy_audit:
                            reason_parts.append("Involved in pairwise noisy label conflicts in audit")
                        ...
                        self.review_actions[tid_str] = ("REVIEW", f"Suspicious label review: {explanation}")
        ...
        if label_review_df.empty:
            label_review_df = pd.DataFrame(columns=["tracking_id", "text", "annotated_class", "suggested_class", "confidence", "explanation"])
        ...
        for cls in classes:
            ...
            if not hard_negatives_df.empty:
                if "class_1" in hard_negatives_df.columns:
                    hard_neg_count = int(((hard_negatives_df["class_1"] == cls) | (hard_negatives_df["class_2"] == cls)).sum())
...
"""
    ...
    def compute_quality_comparison(self, df_orig: pd.DataFrame, df_clean: pd.DataFrame, audit_data: dict, label_review_df: pd.DataFrame) -> tuple[dict, dict, dict]:
        weights = self.config.get("improvement", {}).get("quality_weights", {
            "completeness": 0.20,
            "consistency": 0.20,
            "uniqueness": 0.20,
        ...
        def calculate_scores(data_df: pd.DataFrame, is_before: bool):
            N = len(data_df)
            if N == 0:
                return {"total": 0.0, "completeness": 0.0, "consistency": 0.0, "uniqueness": 0.0, "label_reliability": 0.0, "language_balance": 0.0}
                
            # 1. Completeness: percentage of rows with non-null text and labels
            # Text must also be longer than 50 chars
            invalid_rows = data_df["text"].isna().sum() + (data_df["text"].astype(str).str.len() <= 50).sum()
            ...
            completeness = max(0.0, 100.0 * (1.0 - (invalid_rows / (N * 3)))) # 3 fields evaluated
            ...
            if is_before:
                ...
                noisy_cnt = len(label_review_df[label_review_df["explanation"].str.contains("noisy", case=False)])
            consistency = max(0.0, 100.0 * (1.0 - (noisy_cnt / N)))
            
            # 3. Uniqueness: exact duplicate fraction
            if is_before:
                exact_dup_cnt = int(audit_data.get("duplicates", pd.DataFrame()).get("count", pd.Series([0])).sum())
            ...
            susp_cnt = len(label_review_df) if not is_before else int(len(label_review_df) * 1.5) # approximate before
            ...
            if "language" in data_df.columns:
                ...
                lang_entropy = -float(np.sum(lang_probs * np.log2(lang_probs))) if len(lang_probs) > 0 else 0.0
                max_lang_entropy = np.log2(len(lang_counts)) if len(lang_counts) > 1 else 1.0
        ...
        compare_metrics = {
            "rows": {"before": len(df_orig), "after": len(df_clean), "diff": len(df_clean) - len(df_orig)},
            "missing_values": {"before": int(df_orig["text"].isna().sum() + df_orig["department_code"].isna().sum()), "after": 0, "diff": -int(df_orig["text"].isna().sum())},
            "exact_duplicates": {
                "before": int(audit_data.get("duplicates", pd.DataFrame()).get("count", pd.Series([0])).sum()),
                "after": 0,
                "diff": -int(audit_data.get("duplicates", pd.DataFrame()).get("count", pd.Series([0])).sum())
            },
            "quality_score": {"before": score_before["total"], "after": score_after["total"], "diff": score_after["total"] - score_before["total"]}
        }
        
        return score_before, score_after, compare_metrics
        
    def export_artifacts(self, df: pd.DataFrame, label_review_df: pd.DataFrame, aug_plan: dict, merge_plan: str, compare_metrics: dict, score_before: dict, score_after: dict):
        ...
        if not self.dry_run:
            ...
            self.logger.info(f"Exported dataset_improved.csv to {improved_dataset_path}")
        ...
        if change_log_path.exists() and self.dry_run:
            ...
            if change_log_df.empty:
                change_log_df = pd.DataFrame(columns=["tracking_id", "field_modified", "original_value", "cleaned_value", "operation_applied", "reason"])
        ...
        diff_report_content = self.generate_diff_report(compare_metrics, score_before, score_after, label_review_df)
        ...
        quality_report_content = self.generate_quality_report(score_before, score_after, compare_metrics)
        ...
        manifest = {
            ...
            "metrics": {
                ...
                "quality_score_before": float(compare_metrics["quality_score"]["before"]),
    ...
    def generate_diff_report(self, compare_metrics, score_before, score_after, label_review_df) -> str:
        rows_removed = compare_metrics["rows"]["before"] - compare_metrics["rows"]["after"]
        normalizations_count = len([x for x in self.change_log if x["operation_applied"] == "CLEAN"])
...
"""
    ...
    def generate_quality_report(self, score_before, score_after, compare_metrics) -> str:
...
def main():
    ...
    parser.add_argument("--audit-dir", type=str, help="Override directory containing latest audit artifacts.")
    parser.add_argument("--dry-run", action="store_true", help="Run without writing improved dataset.")

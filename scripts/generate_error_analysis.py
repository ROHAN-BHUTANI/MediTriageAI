...
def main():
    ...
    parser.add_argument(
        "--results-dir",
        type=str,
        help="Path to an experiment results directory (relative or absolute).",
    )
    ...
    if args.results_dir:
        ...
        if not results_base.exists():
            print(
                "Error: No --results-dir provided and 'results' directory not found.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if not directories:
            print(
                f"Error: No experiment directories found in {results_base}.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            f"No --results-dir provided. Automatically selected newest directory: {results_dir.name}"
        )
    ...
    if not predictions_path.exists():
        print(
            f"Error: Required predictions artifact '{predictions_path}' not found in {results_dir}.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    spec_report = classification_report(
        df["ground_truth_department"],
        df["predicted_department"],
        output_dict=True,
        zero_division=0,
    )
    spec_df = (
        pd.DataFrame(spec_report).T.reset_index().rename(columns={"index": "class"})
    )
    save_table(spec_df, "specialist_metrics")

    # 3. Per-severity metrics
    sev_report = classification_report(
        df["ground_truth_severity"],
    ...
    high_conf_incorrect = (
        df[~df["department_correct"]]
        .sort_values("department_confidence", ascending=False)
        .head(50)
    )
    ...
    low_conf_correct = (
        df[df["department_correct"]]
        .sort_values("department_confidence", ascending=True)
        .head(50)
    )
    ...
    spec_labels = sorted(
        set(df["ground_truth_department"].unique())
        | set(df["predicted_department"].unique())
    )
    conf_matrix_spec = confusion_matrix(
        df["ground_truth_department"], df["predicted_department"], labels=spec_labels
    )
    ...
    for idx, true_label in enumerate(spec_labels):
        for jdx, pred_label in enumerate(spec_labels):
            if idx != jdx and conf_matrix_spec[idx, jdx] > 0:
                confused_spec.append(
                    (true_label, pred_label, conf_matrix_spec[idx, jdx])
                )
    confused_spec_df = pd.DataFrame(
        confused_spec, columns=["True", "Predicted", "Count"]
    ).sort_values("Count", ascending=False)
    save_table(confused_spec_df, "confused_specialist_pairs")

    # 11. Most confused severity pairs
    sev_labels = sorted(
        set(df["ground_truth_severity"].unique())
        | set(df["predicted_severity"].unique())
    )
    conf_matrix_sev = confusion_matrix(
        df["ground_truth_severity"], df["predicted_severity"], labels=sev_labels
    )
    confused_sev = []
    ...
    confused_sev_df = pd.DataFrame(
        confused_sev, columns=["True", "Predicted", "Count"]
    ).sort_values("Count", ascending=False)
    ...
    for source in df["dataset_source"].unique():
        ...
        if len(source_df) > 0:
            cm = confusion_matrix(
                source_df["ground_truth_department"],
                source_df["predicted_department"],
                labels=spec_labels,
            )
            plt.figure(figsize=(8, 6))
            sns.heatmap(
                pd.DataFrame(cm, index=spec_labels, columns=spec_labels),
                annot=False,
                cmap="Blues",
            )
    ...
    fp_fn_df = pd.DataFrame(
        fp_fn, columns=["Class", "False_Positives", "False_Negatives"]

...
def train_eval_loop(
    df_train, df_val, df_test, label_col, model_name, patience=3, max_epochs=15
):
    ...
    train_dataset = MedDataset(
        df_train["text"].tolist(), [label2id[l] for l in df_train[label_col]], tokenizer
    )
    val_dataset = MedDataset(
        df_val["text"].tolist(), [label2id[l] for l in df_val[label_col]], tokenizer
    )
    test_dataset = MedDataset(
        df_test["text"].tolist(), [label2id[l] for l in df_test[label_col]], tokenizer
    )
    ...
    optimizer = torch.optim.AdamW(
        [{"params": encoder_params, "lr": 2e-5}, {"params": head_params, "lr": 1e-4}],
        weight_decay=0.01,
    )
    ...
    for epoch in range(max_epochs):
        ...
        for batch in train_loader:
            if first_step:
                print(
                    f"[{model_name}] Step 0 LRs: Encoder={optimizer.param_groups[0]['lr']:.2e}, Classifier={optimizer.param_groups[1]['lr']:.2e}"
                )
                first_step = False
    ...
    dist = {
        id2label[i]: int(sum([p == i for p in all_preds]))

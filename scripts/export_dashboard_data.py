...
def to_dashboard_model(slug: str, metrics: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "specialist_f1": float(
            metrics.get("specialist_macro_f1", metrics.get("specialist_f1", 0.0))
        ),
        "severity_f1": float(
            metrics.get("severity_macro_f1", metrics.get("severity_f1", 0.0))
        ),
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Export MediTriageAI results to dashboard_web/data/results.json."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate the dashboard payload without writing files.",
    )
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=RESULTS_DIR,
        help="Directory containing results/*/metrics.json files.",
    )
    parser.add_argument(
        "--dataset-csv",
        type=Path,
        default=DATASET_CSV,
        help="Processed dataset CSV used for summary statistics.",
    )
    parser.add_argument(
        "--manifest",
        type=Path,

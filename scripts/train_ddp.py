...
def parse_args():
    ...
    parser.add_argument("--mode", choices=["smoke", "development", "publication"], default="development")
...
def build_data_loaders(config: TrainingConfig, rank: int, world_size: int, mode: str):
    ...
    train_loader = DataLoader(
        ...
        pin_memory=config.pin_memory, persistent_workers=config.persistent_workers if num_workers > 0 else False
    )
...
def main():
    ...
    try:
        ...
        train_loader, val_loader, test_loader, tokenizer, train_sampler = build_data_loaders(config, rank, world_size, args.mode)
        ...
        if rank == 0:
            ...
            print(f"Training completed successfully. Throughput: {metrics.samples_per_sec:.2f} samples/sec.")

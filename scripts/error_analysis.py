...
def main():
    ...
    if "department_confidence" in errors_df.columns:
        ...
        errors_df.head(100).to_csv(
            results_dir / "top_100_high_conf_errors.csv", index=False
        )

        errors_df = errors_df.sort_values(by="department_confidence", ascending=True)
        errors_df.head(100).to_csv(
            results_dir / "top_100_low_conf_errors.csv", index=False
        )
    ...
    never_predicted = [
        c for c in classes if c not in pred_dist.index or pred_dist[c] == 0
    ]

    correct_counts = corrects_df["ground_truth_department"].value_counts()
    never_correctly_predicted = [
        c for c in classes if c not in correct_counts.index or correct_counts[c] == 0
    ]
    ...
    for cls in classes:
        ...
        if len(cls_df) > 0:
            acc = (
                cls_df["ground_truth_department"] == cls_df["predicted_department"]
            ).mean()
            conf = (
                cls_df["department_confidence"].mean()
                if "department_confidence" in cls_df.columns
                else 0
            )
            calibration.append(
                {

    ...
    if stats_path.exists():
        try:
            ...
            df = pd.DataFrame([
                ...
                {'Metric': 'With Department', 'Value': stats.get('rows_with_department', 'N/A')},
                {'Metric': 'With Severity', 'Value': stats.get('rows_with_severity', 'N/A')},
                {'Metric': 'With Both', 'Value': stats.get('rows_with_both', 'N/A')},
            ])
        ...
        logger.warning(f"Missing {stats_path}. Please build the dataset. Generating placeholder table.")
...
def generate_model_comparison_table(output_base_path: Path) -> None:
    ...
    if results_path.exists():
        try:
            ...
            df = pd.DataFrame([{'Model': 'Error', 'Spec F1': 'Error', 'Sev F1': 'Error', 'Adj Error': 'Error'}])
    else:
        logger.warning(f"Missing {results_path}. Please run evaluations. Generating placeholder table.")
        df = pd.DataFrame([{
            'Model': 'Baseline', 'Spec F1': 'Pending', 'Sev F1': 'Pending', 'Adj Error': 'Pending'
        }, {
            'Model': 'E-PATH-CO-REASON', 'Spec F1': 'Pending', 'Sev F1': 'Pending', 'Adj Error': 'Pending'
        }])
...
def generate_ablation_table(output_base_path: Path) -> None:
    ...
    # This is a template
    df = pd.DataFrame([
        {'Ablation': 'Full E-PATH-CO-REASON', 'Spec F1': 'Pending', 'Sev F1': 'Pending'},
        {'Ablation': 'w/o Co-reasoning', 'Spec F1': 'Pending', 'Sev F1': 'Pending'},
        {'Ablation': 'w/o Emergent Path', 'Spec F1': 'Pending', 'Sev F1': 'Pending'},
    ])

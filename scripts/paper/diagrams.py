...
def draw_box(ax: plt.Axes, xy: Tuple[float, float], width: float, height: float, text: str, facecolor: str = 'lightblue') -> None:
    ...
    """
    rect = patches.Rectangle(xy, width, height, linewidth=1, edgecolor='black', facecolor=facecolor)
    ax.add_patch(rect)
    ax.text(xy[0] + width/2, xy[1] + height/2, text, horizontalalignment='center', verticalalignment='center', fontsize=10)
...
def generate_architecture_diagram(output_path: Path) -> None:
    ...
    draw_box(ax, (3, 5), 4, 1.5, 'E-PATH-CO-REASON\\n(XLM-RoBERTa)', facecolor='#c8e6c9')
    draw_box(ax, (1, 2), 3, 1.5, 'Specialist Routing\\n(Department)', facecolor='#ffccbc')
    draw_box(ax, (6, 2), 3, 1.5, 'Triage Severity\\n(Level S1-S5)', facecolor='#ffccbc')
    
    # Arrows
    ax.annotate('', xy=(5, 6.5), xytext=(5, 8), arrowprops=dict(arrowstyle='->', lw=1.5))
    ax.annotate('', xy=(2.5, 3.5), xytext=(4, 5), arrowprops=dict(arrowstyle='->', lw=1.5))
    ax.annotate('', xy=(7.5, 3.5), xytext=(6, 5), arrowprops=dict(arrowstyle='->', lw=1.5))
...
def generate_pipeline_diagram(output_path: Path) -> None:
    ...
    stages = ['Ingest', 'Schema Align', 'Normalize', 'Deduplicate', 'Filter', 'Partitions', 'Export']
    for i, stage in enumerate(stages):
        draw_box(ax, (i*2, 1.5), 1.6, 1, stage, facecolor='#bbdefb')
        if i < len(stages) - 1:
            ax.annotate('', xy=(i*2 + 1.8, 2), xytext=(i*2 + 1.6, 2), arrowprops=dict(arrowstyle='->', lw=1.5))

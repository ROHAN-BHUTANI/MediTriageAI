...
class DatasetSummaryAnalyzer(BaseAnalyzer):
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        {k: (v / total_rows) * 100 for k, v in class_counts.items()}
        ...
        imbalance_ratio = (
            max(class_counts.values()) / max(1, min(class_counts.values()))
            if class_counts
            else 1.0
        )
        probs = (
            np.array(list(class_counts.values())) / total_rows
            if total_rows > 0
            else np.array([])
        )
        shannon_entropy = (
            -float(np.sum(probs * np.log2(probs))) if len(probs) > 0 else 0.0
        )
        ...
        try:
            tokenizer = AutoTokenizer.from_pretrained(
                tokenizer_name, local_files_only=False
            )
            logger.info("Loaded tokenizer successfully.")
        except Exception as e:
            logger.warning(
                f"Could not load pre-trained tokenizer: {e}. Using whitespace fallback."
            )
            tokenizer = None
        ...
        if tokenizer is not None:
            try:
                ...
                logger.warning(
                    f"Batch tokenization failed: {e}. Falling back to whitespace counts."
                )
        ...
        pct_64 = (
            float((token_lengths_s > 64).sum() / total_rows * 100)
            if total_rows > 0
            else 0.0
        )
        pct_128 = (
            float((token_lengths_s > 128).sum() / total_rows * 100)
            if total_rows > 0
            else 0.0
        )
        pct_256 = (
            float((token_lengths_s > 256).sum() / total_rows * 100)
            if total_rows > 0
            else 0.0
        )
        pct_512 = (
            float((token_lengths_s > 512).sum() / total_rows * 100)
            if total_rows > 0
            else 0.0
        )
            ...
            [
                {
                    ...
                    "percentage": float(bin_counts.get(label, 0) / total_rows * 100)
                    if total_rows > 0
                    else 0.0,
                }
        ...
        outliers_df = df[(char_lengths < lower_bound) | (char_lengths > upper_bound)][
            ["tracking_id", "text", "department_code"]
        ].copy()
        outliers_df["length"] = outliers_df["text"].astype(str).str.len()
        outliers_df["outlier_reason"] = outliers_df["length"].apply(
            lambda l: "short_outlier" if l < lower_bound else "long_outlier"
        )
        ...
        sev_counts = (
            df["severity_heuristic"].value_counts().to_dict()
        ...
        if label_stats_df.empty:
            label_stats_df = pd.DataFrame(
                columns=["label_type", "label_value", "count", "percentage"]
            )
...
class DuplicateAnalyzer(BaseAnalyzer):
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        if total_rows == 0:
            return {
                "artifacts": {
                    "duplicate_texts.csv": pd.DataFrame(
                        columns=["text", "count", "tracking_ids", "classes"]
                    ),
                    "near_duplicate_texts.csv": pd.DataFrame(
                        columns=[
                            "text_1",
                            "text_2",
                            "similarity",
                ...
                exact_dup_rows.append(
                    {
                        ...
                        "count": len(idxs),
                        "tracking_ids": ",".join(
                            df["tracking_id"].iloc[idxs].astype(str)
                        ),
                        "classes": ",".join(
                            df["department_code"].iloc[idxs].astype(str)
                        ),
                    }
                )
        ...
        try:
            ...
            for r, c, val in zip(row_indices, col_indices, sim_values):
                ...
                if txt1 != txt2:
                    pair_key = tuple(
                        sorted([df["tracking_id"].iloc[r], df["tracking_id"].iloc[c]])
                    )
...
class VocabularyAnalyzer(BaseAnalyzer):
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        return {
            "artifacts": {"vocabulary_statistics.json": vocab_stats},
            "metrics": {
                "vocabulary_size": vocab_size,
                "unique_token_ratio": unique_token_ratio,
            },
    ...
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        for cls in classes:
            ...
            if tokenizer is not None:
                try:
                    token_lengths = [
                        len(ids) for ids in tokenizer(texts, verbose=False)["input_ids"]
                    ]
                except Exception:
            ...
            for words in all_words:
                ...
                for i in range(len(filtered_w) - 2):
                    trigrams.append(
                        f"{filtered_w[i]} {filtered_w[i + 1]} {filtered_w[i + 2]}"
                    )
        ...
        return {
            "artifacts": {"class_token_statistics.json": class_stats},
            "metrics": {},
        }
...
class LanguageAnalyzer(BaseAnalyzer):
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        if total_rows == 0:
            return {
                "artifacts": {
                    "multilingual_statistics.csv": pd.DataFrame(
                        columns=["language_type", "count", "percentage"]
                    )
                },
                "metrics": {
        ...
        return {
            "artifacts": {"multilingual_statistics.csv": multilingual_stats},
            "metrics": metrics,
        }
...
class RegexCoverageAnalyzer(BaseAnalyzer):
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        return {
            "artifacts": {"regex_coverage_statistics.json": coverage_stats},
            "metrics": {},
        }
    ...
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        for category, entities in entity_dict.items():
            for entity in entities:
                ...
                if len(df) > 0:
                    matches = (
                        df["text"].astype(str).apply(lambda t: len(pattern.findall(t)))
                    )
                    total_matches = int(matches.sum())
                else:
                    total_matches = 0
                entity_counts.append(
                    {"entity": entity, "category": category, "frequency": total_matches}
                )
        ...
        return {
            "artifacts": {"medical_entity_frequencies.csv": entity_df},
            "metrics": {},
        }
...
class LeakageAnalyzer(BaseAnalyzer):
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        if "split" not in df.columns:
            logger.warning(
                "No 'split' column found in dataset. Skipping split leakage analysis."
            )
            return {
                "artifacts": {
                    "split_leakage_report.csv": pd.DataFrame(
                        columns=[
                            "text",
                            "source_split",
                            "target_split",
                    ...
                    "split_similarity_matrix.csv": pd.DataFrame(
                        columns=["split", "train", "val", "test"]
                    ),
                },
        ...
        if leakage_report_df.empty:
            leakage_report_df = pd.DataFrame(
                columns=["text", "source_split", "target_split", "type", "similarity"]
            )
        ...
        train_texts = (
            set(df[df["split"] == "train"]["text"].astype(str))
            if "train" in splits
            else set()
        )
        ...
        return {
            ...
            "metrics": {"leakage_percentage": leakage_pct},
        }
    ...
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        try:
            ...
            if not class_sim_matrix_df.empty:
                ...
                ax.set_title(
                    "Class-wise Semantic Similarity Heatmap",
                    fontsize=14,
                    fontweight="bold",
                )
                plt.tight_layout()
            ...
            rows, cols = similarity_matrix.nonzero()
            mask = (similarity_matrix.data >= min_hn) & (
                similarity_matrix.data <= max_hn
            )
            ...
            metrics = {
                ...
                "noisy_label_percentage": (len(noisy_df) / len(df)) * 100
                if len(df) > 0
                else 0.0,
            }
...
class AugmentationAnalyzer(BaseAnalyzer):
    def analyze(self, df: pd.DataFrame, logger) -> dict:
        ...
        noisy_counts = (
            df.groupby("department_code")["noisy_label_count"].first().to_dict()
            if "noisy_label_count" in df.columns
            else {}
        )
        ...
        for cls in classes:
            ...
            lang_counts = (
                cls_df["detected_language_type"].value_counts().to_dict()
                if "detected_language_type" in cls_df.columns
                else {}
            )
            ...
            readiness_score = max(
                0.0, min(100.0, base_score - dup_deduction - noisy_deduction)
            )

            recs = []
            if needs_en:
                recs.append("more English samples")
            if needs_hi:
                recs.append("Hindi samples")
            if needs_hinglish:
                recs.append("Hinglish samples")
            if needs_typos:
                recs.append("typo augmentation")
            if needs_asr:
                recs.append("ASR augmentation")
            if needs_paraphrases:
                recs.append("paraphrases")
        ...
        return {
            "artifacts": {"augmentation_recommendations.json": recommendations},
            "metrics": {},
        }
...
"""
        return {
            "artifacts": {"dataset_merge_recommendations.md": content},
            "metrics": {},
        }
...
class DatasetAuditOrchestrator:
    def __init__(self, config: dict, cli_args: argparse.Namespace):
        ...
        if cli_args.near_duplicate_threshold:
            self.config["thresholds"]["near_duplicate"] = (
                cli_args.near_duplicate_threshold
            )
        ...
        self.timestamp = datetime.now().strftime(
            "%Y%md_%H%M%S"
        )  # Match simple format: YYYYMMDD_HHMMSS
    ...
    def execute(self):
        ...
        for idx, (name, analyzer) in enumerate(self.analyzers, 1):
            ...
            try:
                ...
                self.logger.debug(
                    f"Module '{name}' completed successfully. Generated {len(module_artifacts)} artifacts."
                )
            except Exception as e:
                status = "FAILED"
                error_msg = str(e)
        ...
        for filename, data in all_artifacts.items():
            ...
            try:
                if isinstance(data, pd.DataFrame):
                    data.to_csv(
                        filepath, index=isinstance(data.index, pd.RangeIndex) == False
                    )
                elif isinstance(data, dict):
                    with open(filepath, "w", encoding="utf-8") as f:
                        json.dump(data, f, indent=2)
                elif isinstance(data, (plt.Figure, matplotlib.figure.Figure)):
                    ...
                    self.logger.warning(
                        f"Unsupported artifact type for {filename}: {type(data)}"
                    )
                    continue
        ...
        artifact_checksums["dataset_summary.json"] = calculate_sha256(
            self.run_dir / "dataset_summary.json"
        )
        ...
        artifact_checksums["artifact_index.md"] = calculate_sha256(
            self.run_dir / "artifact_index.md"
        )

        # Generate Markdown Report
        report_content = self.generate_markdown_report(
            all_metrics, failures, all_artifacts
        )
        with open(self.run_dir / "dataset_audit_report.md", "w", encoding="utf-8") as f:
            f.write(report_content)
        generated_paths.append("dataset_audit_report.md")
        artifact_checksums["dataset_audit_report.md"] = calculate_sha256(
            self.run_dir / "dataset_audit_report.md"
        )
        ...
        manifest = {
            ...
            "package_versions": {
                ...
                "scikit-learn": getattr(
                    sys.modules.get("sklearn"), "__version__", "unknown"
                ),
        ...
        for filename in generated_paths + ["audit_manifest.json"]:
            ...
            if src_path.exists():
                try:
                    ...
                    self.logger.error(
                        f"Failed to copy {filename} to latest directory: {e}"
                    )
        ...
        print(
            f"Shannon Entropy    : {all_metrics['shannon_entropy']:.3f} (Max: {np.log2(all_metrics['number_of_classes']):.3f})"
        )
        print(f"Imbalance Ratio    : {all_metrics['imbalance_ratio']:.2f}")
        print(
            f"Exact Duplicates   : {all_metrics['exact_duplicate_count']} ({all_metrics['exact_duplicate_percentage']:.2f}%)"
        )
        print(
            f"Near Duplicates    : {all_metrics['near_duplicate_count']} ({all_metrics['near_duplicate_percentage']:.2f}%)"
        )
        print(
            f"Noisy Labels Count : {all_metrics['noisy_label_count']} ({all_metrics['noisy_label_percentage']:.2f}%)"
        )
        print(
            f"Language Spread    : English: {all_metrics['english_percentage']:.1f}%, Hinglish: {all_metrics['hinglish_percentage']:.1f}%, Code-Switched: {all_metrics['code_switched_percentage']:.1f}%, Hindi: {all_metrics['hindi_percentage']:.1f}%"
        )
        print("========================================\n")
    ...
    def generate_artifact_index(self, generated_paths, failures) -> str:
        ...
        for fn, err in failures.items():
            md += (
                f"| `{fn}` | Analyzer failed during generation | - | FAILED ({err}) |\n"
            )
...
"""
        ...
        if isinstance(aug_recs, dict):
            for cls_name, item in sorted(aug_recs.items()):
                recs_str = (
                    ", ".join(item.get("recommended_augmentations", []))
                    if item.get("recommended_augmentations")
                    else "None (Ready)"
                )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, help="Override output base directory."
    )
    parser.add_argument(
        "--near-duplicate-threshold",
        type=float,
        help="Override near-duplicate threshold.",
    )
    parser.add_argument(
        "--noisy-label-threshold", type=float, help="Override noisy label threshold."
    )
    ...
    if config_path.exists():
        ...
        logger.warning(
            f"Config file not found at: {args.config}. Using CLI/code defaults."
        )

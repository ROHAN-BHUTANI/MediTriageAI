...
def main():
    ...
    for adapter in adapters:
        ...
        if not adapter.data_path.exists():
            print(
                f"[{name}] WARNING: Path {adapter.data_path} does not exist. Skipping."
            )
            continue
        ...
        try:
            ...
            print(
                f"[{name}] OK - Parsed {record_count} records (capped at 1000 for validation)."
            )
            print(f"[{name}] Splits found: {splits_seen}")
            if null_issues > 0 or schema_issues > 0:
                print(
                    f"[{name}] FAILED - Null text: {null_issues}, Schema Issues: {schema_issues}"
                )
                all_passed = False

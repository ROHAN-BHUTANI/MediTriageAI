...
from ref.benchmarks.strategies import AblationBenchmark, BaselineBenchmark
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="campaign_config.json",
        help="Path to JSON campaign configuration",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Build the execution graph but do not run",
    )
    parser.add_argument(
        "--smoke-test",
        action="store_true",
        help="Run 1 seed on subset of data for validation",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from campaign_state.json if interrupted",
    )
    return parser
...
def main():
    ...
    prov_config = {
        "enable_fingerprint_git": True,
        "enable_fingerprint_environment": True,
    }

...
class EmergentPathTriageModel(BaseMediTriageModel):
    ...
    paper_name: ClassVar[str] = (
        "E-PATH-CO-REASON: Emergent Path-Aligned Co-evolutionary Triage Network"
    )
    ...
    def build_tokenizer(cls):
        logger.info(
            f"Building tokenizer for E-PATH-CO-REASON using model name '{cls.model_name}'"
        )
    ...
    def build_encoder(cls, config: Any | None = None) -> XLMRobertaModel:
        logger.info(
            f"Building encoder for E-PATH-CO-REASON using model name '{cls.model_name}'"
        )
        return AutoModel.from_pretrained(cls.model_name)

    def build(
        self, config: Any, triage_config: EmergentPathTriageConfig | None = None
    ) -> nn.Module:
        ...
        triage_cfg = triage_config or (
            ...
            if isinstance(config, dict)
                ...
                config
                if isinstance(config, EmergentPathTriageConfig)
                else EmergentPathTriageConfig()
            )
        )
...
class EmergentPathTriageTransformer(BaseEmergentPathTriage):
    ...
    def __init__(
        self, encoder: XLMRobertaModel, config: EmergentPathTriageConfig
    ) -> None:
        super().__init__()
        ...
        with torch.amp.autocast(device_type=device_type, enabled=False):
            ...
            if closed_loop:
                ...
                for _t in range(depth):
                    step_output = self.router.step(
                        h_t, router_state, self.config.temperature
                    )
                    ...
                    selected_blocks_list.append(
                        int(step_output.selected_blocks[0].item())
                    )
                ...
                if getattr(self.config, "ablation_router_enabled", True):
                    ...
                    probs = (
                        torch.ones(batch_size, depth, num_blocks, device=device)
                        / num_blocks
                    )
                ...
                final_state, thought_path = self.engine(
                    evidence_list, routing_decision, self.blocks
                )
    ...
    def compute_loss(
        ...
        if len(specialist_logits.shape) != 2 or specialist_logits.shape[1] != 13:
            raise InterfaceError(
                f"Incorrect specialist_logits shape: {specialist_logits.shape}"
            )
        if len(severity_logits.shape) != 2 or severity_logits.shape[1] != 5:
            raise InterfaceError(
                f"Incorrect severity_logits shape: {severity_logits.shape}"
            )
        ...
        if (
            self.config.ortho_lambda > 0.0
            and hasattr(self, "_last_evidence")
        ...
        if (
            self.config.cons_lambda > 0.0
            and hasattr(self, "_last_final_state")
            and self._last_final_state is not None
        ):
        ...
        if (
            self.config.div_lambda > 0.0
            and hasattr(self, "_last_routing_decision")
            and self._last_routing_decision is not None
        ):
        ...
        if hasattr(joint_loss_fn, "specialist_cross_entropy") and hasattr(
            joint_loss_fn, "severity_cross_entropy"
        ):
            raw_specialist_loss = joint_loss_fn.specialist_cross_entropy(
                specialist_logits, labels_specialist
            )
            raw_severity_loss = joint_loss_fn.severity_cross_entropy(
                severity_logits, labels_severity
            )
        else:
            # Fallback if a different loss fn is used, use F.cross_entropy
            raw_specialist_loss = torch.nn.functional.cross_entropy(
                specialist_logits, labels_specialist
            )
            raw_severity_loss = torch.nn.functional.cross_entropy(
                severity_logits, labels_severity
            )
    ...
    def initialize_weights(self) -> None:
        ...
        logger.info(
            f"Initializing E-PATH-CO-REASON head weights with seed={self.routing_seed}"
        )
    ...
    def load_state_dict(
        ...
        has_balancer_keys = any("loss_balancer." in k for k in state_dict)
        ...
        has_spec_calib = any("specialist_calibrator." in k for k in state_dict)
        has_sev_calib = any("severity_calibrator." in k for k in state_dict)
    ...
    def load_checkpoint_metadata(self, path: Path) -> dict[str, Any]:
        ...
        if not meta_file.exists():
            raise CompatibilityError(
                f"Checkpoint metadata file not found at: {meta_file}"
            )

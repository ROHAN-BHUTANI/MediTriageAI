...
class BaseLossBalancer(nn.Module, ABC):
    ...
    def _collect_losses(
        ...
        for task in self.task_names:
            if task not in losses:
                raise ValueError(
                    f"Expected loss for task '{task}', but missing from input."
                )
        return {task: losses[task] for task in self.task_names}

    @abstractmethod
    def _extract_statistics(
        self, loss_tensors: dict[str, torch.Tensor]
    ) -> dict[str, torch.Tensor]:
        pass

    @abstractmethod
    def _apply_strategy(
        self, loss_tensors: dict[str, torch.Tensor], stats: dict[str, torch.Tensor]
    ) -> dict[str, Any]:
        pass

    @abstractmethod
    def _generate_weights(
        self, strategy_output: dict[str, Any]
    ...
    def _record_telemetry(
        self, weights: dict[str, torch.Tensor], strategy_output: dict[str, Any]
    ) -> None:
        pass
...
class StaticLossBalancer(BaseLossBalancer):
    ...
    def _generate_weights(
        ...
        device = (
            next(iter(strategy_output.values())).device if strategy_output else "cpu"
        )
        weights = {
            task: torch.tensor(self.fixed_weights[task], device=device)
            for task in self.task_names
        }
        ...
        if self.recorder.record_enabled:
            trace = OptimizationReasoningTrace(
                ...
                effective_task_weights={
                    k: v.detach().clone() for k, v in weights.items()
                },
            )
...
class HomoscedasticBalancer(BaseLossBalancer):
    ...
    def _record_telemetry(
        ...
        if self.recorder.record_enabled:
            log_vars = strategy_output["log_vars"]
            uncertainties = {
                task: log_vars[task].detach().clone() for task in self.task_names
            }

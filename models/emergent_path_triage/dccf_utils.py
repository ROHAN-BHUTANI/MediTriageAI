...
class ClinicalConfidenceDiagnostics:
    ...
    def aggregate_statistics(
        trace: ClinicalConfidenceTrace, labels: torch.Tensor | None = None
    ) -> dict[str, float]:
        """
    ...
    def compute_calibration_metrics(
        probs: torch.Tensor, labels: torch.Tensor, num_bins: int = 10
    ) -> dict[str, float]:
        """
        ...
        one_hot = torch.nn.functional.one_hot(
            labels_cpu, num_classes=probs_cpu.shape[-1]
        ).float()
        ...
        nll = torch.nn.functional.cross_entropy(
            torch.log(probs_cpu + 1e-10), labels_cpu
        ).item()
        metrics["nll"] = nll

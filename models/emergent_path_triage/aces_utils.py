...
class EvidenceDiagnostics:
    ...
    def aggregate_statistics(trace: EvidenceReasoningTrace) -> Dict[str, Any]:
        ...
        if trace.fusion_type != "AttentionFusion":
            return {"fusion_type": trace.fusion_type, "message": "Detailed diagnostics require AttentionFusion"}
        ...
        if trace.aspect_importance_scores is not None:
            ...
            stats["evidence_confidence_mean"] = float(stacked.max(dim=1).values.mean().item())
        ...
        if trace.interaction_weights is not None:
            ...
            if H > 1:
                ...
                norms = torch.norm(flat_weights, p=2, dim=2, keepdim=True).clamp(min=1e-8)

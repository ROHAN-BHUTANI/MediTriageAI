...
from ref.benchmarks.types import BenchmarkRun, BenchmarkComparison, AblationSummary, AblationRun, AblationDefinition
...
class DummyStrategyMixin:
    ...
    def report(self, aggregated_results: dict[str, Any]) -> BenchmarkComparison | AblationSummary:
        ...
        br_cand = BenchmarkRun(experiment_id="run_candidate", metrics={"accuracy": 0.95})
...
class AblationBenchmark(DummyStrategyMixin, BaseBenchmarkStrategy):
    ...
    def report(self, aggregated_results: dict[str, Any]) -> AblationSummary:
        ...
        br_base = BenchmarkRun(experiment_id="run_baseline", metrics={"accuracy": 0.95})
        ab_def = AblationDefinition(ablation_name="remove_module_x", removed_components=["module_x"])
        ab_run = AblationRun(experiment_id="run_ablation", ablation=ab_def, metrics={"accuracy": 0.90})

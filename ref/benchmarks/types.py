...
class BenchmarkDefinition:
    ...
    def validate(self) -> None:
        if not self.name or not self.target_metric:
            raise BenchmarkValidationError("BenchmarkDefinition must have a name and target metric.")
...
class BenchmarkComparison:
    ...
    def validate(self) -> None:
        if not self.comparison_id:
            raise BenchmarkValidationError("BenchmarkComparison must have a comparison_id.")
    ...
    def from_dict(cls, data: dict[str, Any]) -> "BenchmarkComparison":
        ...
        candidate_runs = [BenchmarkRun.from_dict(r) for r in data.get("candidate_runs", [])]
        return cls(
...
class BenchmarkSuite:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "BenchmarkSuite":
        definitions = [BenchmarkDefinition.from_dict(d) for d in data.get("definitions", [])]
        return cls(
...
class AblationDefinition:
    ...
    def validate(self) -> None:
        if not self.ablation_name:
            raise BenchmarkValidationError("AblationDefinition must have an ablation_name.")
...
class AblationSummary:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "AblationSummary":
        ...
        ablation_runs = [AblationRun.from_dict(r) for r in data.get("ablation_runs", [])]
        return cls(
...
class BenchmarkSummary:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if hasattr(self, "summary_hash"):
            d["summary_hash"] = getattr(self, "summary_hash")
        return _deterministic_dict(d)

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "BenchmarkSummary":
        comparisons = [BenchmarkComparison.from_dict(c) for c in data.get("comparisons", [])]

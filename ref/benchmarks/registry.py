...
class BenchmarkRegistry:
    ...
    def discover_strategies(
        self, strategy_classes: list[type[BaseBenchmarkStrategy]]
    ) -> None:
    ...
    def register(self, suite_name: str, strategy_names: list[str]) -> None:
        ...
        for s_name in strategy_names:
            strategy_enabled = self.config.get(
                f"enable_strategy_{s_name.lower()}", True
            )

            if not strategy_enabled:
                logger.debug(
                    f"Skipping registration of strategy {s_name} (disabled via config)."
                )
                continue

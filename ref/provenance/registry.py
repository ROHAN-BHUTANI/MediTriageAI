...
class ProvenanceRegistry:
    ...
    def discover_providers(
        self, provider_classes: list[type[BaseFingerprintProvider]]
    ) -> None:
    ...
    def register(self, provider_names: list[str]) -> None:
        ...
        for p_name in provider_names:
            provider_enabled = self.config.get(
                f"enable_fingerprint_{p_name.lower()}", True
            )

            if not provider_enabled:
                logger.debug(
                    f"Skipping registration of provenance provider {p_name} (disabled via config)."
                )
                continue

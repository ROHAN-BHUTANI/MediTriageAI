    ...
    def validate(self) -> None:
        if not self.timestamp:
            raise ProvenanceValidationError(
                "ExecutionEnvironment must have a timestamp."
            )
...
class DatasetFingerprint:
    ...
    def validate(self) -> None:
        if not self.dataset_name or not self.checksum:
            raise ProvenanceValidationError(
                "DatasetFingerprint must have name and checksum."
            )
...
class ConfigurationFingerprint:
    ...
    def validate(self) -> None:
        if not self.config_hash:
            raise ProvenanceValidationError(
                "ConfigurationFingerprint must have config_hash."
            )
...
class ExperimentFingerprint:
    ...
    def validate(self) -> None:
        if not self.provider_name:
            raise ProvenanceValidationError(
                "ExperimentFingerprint must have a provider_name."
            )
...
class ProvenanceManifest:
    ...
    def validate(self) -> None:
        if not self.experiment_id:
            raise ProvenanceValidationError(
                "ProvenanceManifest must specify experiment_id."
            )
        for k, v in self.fingerprints.items():
            if not isinstance(v, ExperimentFingerprint):
                raise ProvenanceValidationError(
                    f"Key {k} must map to an ExperimentFingerprint."
                )
            v.validate()
    ...
    def to_dict(self) -> dict[str, Any]:
        d = {
            ...
            "fingerprints": _deterministic_dict(
    ...
    def from_dict(cls, data: dict[str, Any]) -> "ProvenanceManifest":
        ...
        obj = cls(
            ...
            fingerprints={
                k: ExperimentFingerprint.from_dict(v) for k, v in fingerprints.items()
            },
        )

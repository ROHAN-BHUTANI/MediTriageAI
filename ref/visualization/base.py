...
class BaseVisualizationProvider(ABC):
    ...
    def _generate_deterministic_filename(
        self, request: VisualizationRequest, plot_name: str, format: str
    ) -> str:
    ...
    def export(
        self, rendered_objects: dict[str, Any], request: VisualizationRequest
    ) -> dict[str, VisualizationArtifact]:
        """
            ...
            for fmt in formats:
                ...
                if not file_path.exists():
                    file_path.write_text(
                        f"Dummy plot content for {name} in format {fmt}"
                    )
    ...
    def report(
        ...
        collection = VisualizationCollection(
            provider_name=meta.name, artifacts=exported_artifacts
        )
        collection.validate()
        return collection

...
class CampaignRunner:
    ...
    def execute_campaign(
        ...
        if resume and self.state_file.exists():
            ...
            logger.info(
                f"Resuming campaign from {self.state_file}. {len(self._state['completed_runs'])} completed runs found."
            )
        else:
        ...
        logger.info(
            f"Campaign '{campaign_name}' scheduled for {total_runs} total runs."
        )
        ...
        for exp in experiments:
            for seed in seeds:
                ...
                if run_key in self._state["completed_runs"]:
                    logger.info(
                        f"[{run_idx}/{total_runs}] Skipping {run_key} (already completed)."
                    )
                    continue
                ...
                try:
                    ...
                    self._save_state()
                    logger.info(
                        f"[{run_idx}/{total_runs}] Successfully completed {run_key}."
                    )
                    ...
                    logger.warning(
                        "Continuing to next scheduled run to prevent campaign collapse."
                    )

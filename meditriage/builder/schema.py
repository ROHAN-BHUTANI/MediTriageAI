...
def validate_schema(df: pd.DataFrame, require_split: bool = False) -> None:
    ...
    for col in strict_non_null:
        if df[col].isnull().any():
            ...
            raise ValueError(
                f"Schema validation failed: Column '{col}' contains {null_count} null values."
            )
    ...
    if invalid_conf:
        raise ValueError(
            f"Schema validation failed: Invalid routing_confidence values {invalid_conf}"
        )
    ...
    if invalid_sev:
        raise ValueError(
            f"Schema validation failed: Invalid triage_level values {invalid_sev}"
        )
    ...
    if require_split:
        ...
        if invalid_splits:
            raise ValueError(
                f"Schema validation failed: Invalid split values {invalid_splits}"
            )

...
class ClinicalRuleEngine:
    ...
    DEPT_COMPATIBILITY = {
        "CARDIO_PULM": ["chest", "heart", "breath", "lung", "cardiac", "cough", "wheezing", "edema", "palpitation", "dyspnea", "pain", "substernal", "pressure", "discomfort"],
        "NEURO": ["head", "brain", "headache", "seizure", "stroke", "weakness", "numbness", "dizziness", "droop", "aphasia"],
        "ORTHO": ["bone", "joint", "fracture", "sprain", "dislocation", "swelling", "leg", "arm", "wrist", "ankle", "trauma", "pain"],
        "PEDS": ["child", "pediatric", "fever", "cough", "vomiting", "feeding", "infant", "toddler", "crying", "lethargy"],
        "ENT": ["ear", "nose", "throat", "hearing", "discharge", "sinus", "tonsil", "otalgia", "pain"],
        "OBGYN": ["pregnancy", "labor", "vaginal", "pelvic", "maternal", "uterine", "fetal", "cramp"],
    }
    ...
    def validate_clinical_rules(
        ...
        for contra in phenotype.contraindicated_symptoms:
            if contra.lower() in text_lower:
                return False, f"Contraindicated symptom '{contra}' present for phenotype '{phenotype.name}'"
        ...
        for pat1, pat2 in self.CONTRADICTORY_PAIRS:
            if re.search(pat1, text_lower) and re.search(pat2, text_lower):
                return False, f"Contradictory clinical pair matched: '{pat1}' and '{pat2}'"
        ...
        if dept_key in self.DEPT_COMPATIBILITY:
            ...
            if not any(kw in text_lower for kw in valid_keywords):
                return False, f"Text lacks organ system keywords compatible with department '{dept_key}'"
        ...
        if triage_level in ("S1", "S2"):
            ...
            if any(t in text_lower for t in trivial_only):
                return False, f"Trivial symptom inconsistent with high-acuity triage level '{triage_level}'"

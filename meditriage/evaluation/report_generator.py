...
class PublicationReportGenerator:
    ...
    def generate_all_publication_reports(
        ...
        with open(self.output_dir / "ablation_results.json", "w", encoding="utf-8") as f:
        ...
        latex_full = f"% MediTriageAI LaTeX Tables\n\n{latex_ablation}\n\n{latex_backbone}\n"
        with open(self.output_dir / "latex_tables.tex", "w", encoding="utf-8") as f:
            f.write(latex_full)

        # 3. publication_tables.md
        pub_tables_md = self._build_publication_tables_md(ablation_results, backbone_results)
        with open(self.output_dir / "publication_tables.md", "w", encoding="utf-8") as f:
        ...
        with open(self.output_dir / "statistical_significance.md", "w", encoding="utf-8") as f:
            f.write(sig_md)
...
"""
        ...
        logger.info("All publication research artifacts successfully generated in %s", self.output_dir)
        return master_output

    def _build_publication_tables_md(
        self, ablation_results: list[dict[str, Any]], backbone_results: list[dict[str, Any]]
    ) -> str:
        md = ["# MediTriageAI Publication Tables\n", "## Table 1: Ablation Study Results\n"]
        md.append("| Experiment Variant | Accuracy | Macro F1 | Weighted F1 | Balanced Acc. |")
        md.append("| :--- | :---: | :---: | :---: | :---: |")
...
"""
    ...
    def _build_statistical_significance_md(self, significance_results: dict[str, Any]) -> str:
